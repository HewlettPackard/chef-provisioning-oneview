...
RSpec.describe Chef::Provisioning::OneViewDriver do
  ...
  describe '#get_icsp_server_by_sn' do
    it 'gets info for a valid SN' do
      ret_val = @driver_120.instance_eval { get_icsp_server_by_sn('VCGE9KB041') }
    ...
    it 'returns nil for fake SN' do
      ret_val = @driver_120.instance_eval { get_icsp_server_by_sn('FAKESN') }
      expect(ret_val).to be_nil
    end

    it 'fails when an empty SN is given' do
      expect { @driver_120.instance_eval { get_icsp_server_by_sn('') } }.to raise_error('Must specify a serialNumber!')
    end
